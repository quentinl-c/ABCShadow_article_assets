...
class IsingGraphModelNG(GraphModel):
    """
    ...
    def evaluate(self, sample):
        ...
        stats = sample.get_ising_ng_stats()
        return np.dot(self._params, stats)
    ...
    def get_local_energy(self, sample, node):
        ...
        interactions_count = sample.get_ising_ng_local_stats(node)
    ...
    def get_local_stats(self, sample, node):
        ...
        return sample.get_ising_ng_local_stats(node)
    ...
    def get_gibbs_proposal(self, sample, node):
        ...
        res = np.zeros(2)
        stats = sample.get_gibbs_ng_stats_proposals(node)
    ...
    def get_delta_stats(mut_sample, node, new_label):
        ...
        """
        return mut_sample.ising_ng_change_stats(node, new_label)
    ...
    def get_stats(sample):
        ...
        """
        return sample.get_ising_ng_stats()
    ...
    def summary_dict(samples):
        ...
        res = np.array([g.get_ising_ng_stats() for g in samples])
        data['theta01'] = res[:, 0]
        data['theta02'] = res[:, 1]
        data['theta12'] = res[:, 2]
