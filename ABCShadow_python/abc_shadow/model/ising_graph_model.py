...
class IsingGraphModel(GraphModel):
    """
    ...
    def get_local_energy(self, sample, node):
        ...
        interactions_count = sample.get_local_interaction_count(node, 3)
    ...
    def get_local_stats(self, sample, node):
        ...
        return sample.get_local_interaction_count(node, 3)
    ...
    def get_delta_stats(mut_sample, node, new_label):
        ...
        """
        return mut_sample.get_local_interaction_diff(node, 3, new_label)
    ...
    def summary_dict(samples):
        ...
        res = np.array([g.get_interactions_count(3) for g in samples])
        data['theta01'] = res[:, 0]
        data['theta02'] = res[:, 1]
        data['theta12'] = res[:, 2]
